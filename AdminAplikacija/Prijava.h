...
namespace AdministrativnaAplikacija {
	...
	/// Summary for Prijava
	/// </summary>
	public ref class Prijava : public System::Windows::Forms::Form
	{
	public:
		Prijava(void)
		{
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->textBox1->TextChanged += gcnew System::EventHandler(this, &Prijava::textBox1_TextChanged);
			// 
			// label2
			// 
			...
			this->Name = L"Prijava";
			this->Text = L"Prijava";
			this->Load += gcnew System::EventHandler(this, &Prijava::Prijava_Load);
			this->ResumeLayout(false);
			this->PerformLayout();

		}
#pragma endregion
	private: System::Void Prijava_Load(System::Object^  sender, System::EventArgs^  e) {
	}
	...
	private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
		...
		std::string userNameUnmenaged = static_cast<const char*>(Marshal::StringToHGlobalAnsi(userName).ToPointer());	
		std::string passwordUnmenaged = static_cast<const char*>(Marshal::StringToHGlobalAnsi(password).ToPointer());
		std::string stringTemp;

		std::ifstream inFile;
		inFile.open("../../Fajlovi\\RegistrovaniKorisnici.txt");

		if (!inFile) {
			MessageBox::Show("Greska pri otvaranju datoteke");
		}
		else
		{
			int i = 0, j = 0;
			while (inFile >> stringTemp)
			{
				if (stringTemp == userNameUnmenaged)
				{
					inFile >> stringTemp;
					if (stringTemp == passwordUnmenaged)
						j = 1;
				}
			}
			if (j)
